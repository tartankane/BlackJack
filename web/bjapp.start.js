function startGame() {
	...
	$.getJSON(
			...
			function(data) {
				// console.log(data);
				var index1, index2, backOfCard, cardImage1, cardImage2, startGameButton, hitPlayerButton, playerStandsButton, dealerMessage, playerMessage, playerDoublesButton;
				...
				for (index1 = 0; index1 < data.dealerCards.length; index1++) {
					cardImage1 = "images/"
							+ data.dealerCards[index1].rank
							+ data.dealerCards[index1].suit + ".png";
					$('#dealercards').append(
							$('<img src= ' + cardImage1 + '>').fadeIn(
									2000));
				}
				...
				for (index2 = 0; index2 < data.playerCards.length; index2++) {
					cardImage2 = "images/"
							+ data.playerCards[index2].rank
							+ data.playerCards[index2].suit + ".png";
					$('#playercards').append(
							$('<img src= ' + cardImage2 + '>').fadeIn(
									2000));
				}

				/*
				 * make the start button invisible and make the buttons
				 * for the next stage of the game visible and clickable
				 */
...
//				playerDoublesButton.style.visibility = 'visible';
